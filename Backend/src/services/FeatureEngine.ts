import type { ISessionFeatures, IPrediction, IAlerts, IChildControls } from '../interfaces/index.js';
...
const sortedByStart = (sessions: SessionSlice[]): SessionSlice[] =>
    [...sessions].sort(
        (a, b) => new Date(a.start).getTime() - new Date(b.start).getTime(),
    );

/** Gap in minutes between session A's end and session B's start */
const gapMinutes = (a: SessionSlice, b: SessionSlice): number =>
    (new Date(b.start).getTime() - new Date(a.end).getTime()) / 60_000;

/** Hour (0-23) that a session started in IST (Asia/Kolkata timezone) */
const startHour = (s: SessionSlice): number => {
...
const istDayKey = (dateInput: Date | string): string => {
 ...
 */
const computeTrend = (
    allSessionsThisWeek: SessionSlice[],
 ...
 *
 * @param current       The session just completed: { start, end, duration }
 * @param todaySessions All sessions today (NOT including current)
 * @param weekSessions  Last 7 days of sessions (NOT including current)
 */
export function computeFeatures(
    current: SessionSlice,
    todaySessions: SessionSlice[] = [],
    ...
    // ── 2. shortSessionRatio ─────────────────────────────────────────────
    const shortCount = allToday.filter(
        (s) => s.duration < SHORT_SESSION_THRESHOLD_MIN,
    ).length;
    const shortSessionRatio = parseFloat(
        (shortCount / allToday.length).toFixed(2),
    );
    ...
    const interSessionGap =
        gaps.length > 0
            ? parseFloat(
                  (gaps.reduce((s, g) => s + g, 0) / gaps.length).toFixed(2),
              )
            : 0;
...
export function evaluateAlerts(
    ...
    controls: Partial<IChildControls> = {},
): IAlerts {
    ...
    if (
        ...
        features.dailyTotalTime > controls.dailyLimitMinutes
    ) {
