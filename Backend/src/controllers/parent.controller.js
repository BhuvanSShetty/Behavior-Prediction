...
export const getChildDashboard = async (req, res) => {
    try {
        ...
        // Pull dailyTotalTime from the last computed features doc (most accurate)
        const latestSession = todaySessions[todaySessions.length - 1];
        const todayPlayTime = latestSession?.features?.dailyTotalTime
            ?? todaySessions.reduce((sum, s) => sum + (s.raw?.duration || 0), 0);
